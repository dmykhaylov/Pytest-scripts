...
class TestRegisterForm:
    ...
    def test_new_quote_change_type_of_shipment(self, browser):
        button = browser.find_element(By.ID, "mat-radio-11")
        button.click()
        button.click()
    ...
    def test_new_quote_fill_freight_class_form(self, browser):
        time.sleep(2)
        freight_class = browser.find_element(By.ID, "mat-select-2")
        freight_class.click()
        f_class = browser.find_elements(By.CLASS_NAME, "mat-option-text")
        for i in f_class:
            if i.text == user_info['freight_class']:
                i.click()
                break

    def test_new_quote_check_high_value(self, browser):
        check = browser.find_element(By.CLASS_NAME, "bold")
        check.click()

    def test_new_quote_enter_dollar_value(self, browser):
        time.sleep(1)
        dollar_value = browser.find_element(By.XPATH, "//*[@placeholder='Enter Dollar Value']")
        dollar_value.send_keys(user_info['dollar_value'])

    def test_new_quote_enter_pallets(self, browser):
        time.sleep(1)
        pallets = browser.find_element(By.XPATH, "//*[@placeholder='# of Pallets']")
        pallets.send_keys(user_info['pallets'])

    def test_new_quote_enter_length(self, browser):
        time.sleep(1)
        length = browser.find_element(By.XPATH, "//*[@placeholder='Length (in)']")
        length.send_keys(user_info['length'])

    def test_new_quote_enter_width(self, browser):
        time.sleep(1)
        width = browser.find_element(By.XPATH, "//*[@placeholder='Width (in)']")
        width.send_keys(user_info['width'])

    def test_new_quote_enter_height(self, browser):
        time.sleep(1)
        height = browser.find_element(By.XPATH, "//*[@placeholder='Height (in)']")
        height.send_keys(user_info['height'])

    def test_get_new_palletized_item(self, browser):
        button = browser.find_element(By.CLASS_NAME, "ml-20.mat-raised-button.mat-primary.ng-star-inserted")
        button.click()
        time.sleep(3)

    def test_delete_palletized_item(self, browser):
        button = browser.find_element(By.CLASS_NAME, "s-20.mat-icon.notranslate.material-icons.mat-icon-no-color")
        button.click()
    ...
    def test_details_location_name_pickup(self, browser):
        ...
        location_name.send_keys('games-workshop')
    ...
    def test_details_location_name_drop(self, browser):
        ...
        location_name[1].send_keys('zy houmie')
    ...
    def test_details_commodity(self, browser):
        ...
        commodity[1].send_keys('My test hERe')
